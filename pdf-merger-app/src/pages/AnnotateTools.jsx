  ...
  const addPageNumbers = async () => {
    ...
    try {
      ...
      pages.forEach((page, i) => {
        ...
        page.drawText(`${i + 1} / ${pages.length}` , { x: width - 80, y: 24, size: 10, font, color: rgb(0.3,0.3,0.3) })
      })
            ...
            <div className="futuristic-card" style={{ padding: 16 }}>
              ...
              <div className="features-actions" style={{ marginTop: 12 }}>
                <button className="action-card futuristic-card" onClick={addPageNumbers} disabled={!pdfFile || isBusy}>
                  <div className="action-content"><div className="action-text"><h4>{isBusy ? 'Working…' : 'Add Page Numbers'}</h4><p>Numbers bottom-right of each page</p></div></div>
                  <span className="action-indicator">Download →</span>
                </button>
                <button className="action-card futuristic-card" onClick={() => addWatermark()} disabled={!pdfFile || isBusy}>
